...
uint8_t IMU_RxBuf_Wit[IMU_DATASIZE - 1] = { 0 };
...
/**
	...
	* @retval None
*/
void IMU_UartInit_WIT(void)
{
	__HAL_UART_ENABLE_IT(&huart1, UART_IT_IDLE);
	HAL_UART_Receive_DMA(&huart1, IMU_RxBuf_Wit, IMU_DATASIZE);
}
...
*/
void IMU_Data_Handler_WIT(void)
{
	for (int8_t i = 0; i < 3; i++)
	{
		if (IMU_RxBuf_Wit[11 * i] != IMU_HEAD)
		{
			return;
		}
		switch (IMU_RxBuf_Wit[i * 11 + 1])
		{
		case IMU_SPEED:
			speed[0] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 3] << 8) | IMU_RxBuf_Wit[i * 11 + 2]) * 2000.0f / 32768.0f;
			speed[1] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 5] << 8) | IMU_RxBuf_Wit[i * 11 + 4]) * 2000.0f / 32768.0f;
			speed[2] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 7] << 8) | IMU_RxBuf_Wit[i * 11 + 6]) * 2000.0f / 32768.0f;
			Pih_Speed = speed[0] / 6.0f;
			Yaw_Speed = speed[2] / 6.0f;
			break;
		case IMU_ANGLE:
			angle[0] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 3] << 8) | IMU_RxBuf_Wit[i * 11 + 2]) * 180.0f / 32768.0f;
			angle[1] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 5] << 8) | IMU_RxBuf_Wit[i * 11 + 4]) * 180.0f / 32768.0f;
			angle[2] = (short)(((short)IMU_RxBuf_Wit[i * 11 + 7] << 8) | IMU_RxBuf_Wit[i * 11 + 6]) * 180.0f / 32768.0f;
			...
			break;
		case IMU_QUATERNION:
			quaternion.w = (short)(((short)IMU_RxBuf_Wit[i * 11 + 3] << 8) | IMU_RxBuf_Wit[i * 11 + 2]) / 32768.0f;
			quaternion.x = (short)(((short)IMU_RxBuf_Wit[i * 11 + 5] << 8) | IMU_RxBuf_Wit[i * 11 + 4]) / 32768.0f;
			quaternion.y = (short)(((short)IMU_RxBuf_Wit[i * 11 + 7] << 8) | IMU_RxBuf_Wit[i * 11 + 6]) / 32768.0f;
			quaternion.z = (short)(((short)IMU_RxBuf_Wit[i * 11 + 9] << 8) | IMU_RxBuf_Wit[i * 11 + 8]) / 32768.0f;
			break;
		}
	}
}
