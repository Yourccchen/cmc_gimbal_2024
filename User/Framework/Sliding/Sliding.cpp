...
void cSMC::ErrorUpdate(float target, float pos_now, float vol_now) //误差更新
{
    smc.error.tar_now = target;
    smc.error.tar_differential = (float)((smc.error.tar_now - smc.error.tar_last)/0.002);
...
float cSMC::SmcCalculate()
{
    ...
    fun = smc.epsilon * Sat(s);//饱和函数消除抖动
